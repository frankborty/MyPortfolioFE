...
interface IncomeTypeSummary {
  ...
  average: number;
  count: number;
  monthIncome: [number, number, number, number, number, number, number, number, number, number, number, number];
  description: string;
}
...
export class IncomeSummaryTableComponent implements OnInit, OnChanges {
  ...
  processInputData() {
    ...
    filteredByYear.forEach((currentIncome) => {
      const existingItem = this.incomeTypeSummaryList.find(item => item.name === currentIncome.incomeType.name);
      if(existingItem){
    ...
    console.info(this.incomeTypeSummaryList);
  }
